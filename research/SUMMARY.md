...
>
> **Correction and final status after session 26 (2026-09-08).** The session-19 claim immediately
> above — "no named family or sub-mechanism without coverage" — was **wrong when written**, and
> session 25 found out how: it was true of the family *headings* and false of the sub-mechanism
> *clauses* inside them. A direct search across all 76 notes then in the folder found
> **cointegration** and **hierarchical risk parity** with zero coverage while the lab was closing
> their families on entirely different objects. Session 25 covered both; session 26 covered the last
> one it named, `portfolio-learning`'s **stacking / meta-labelling** clause. **Every named family
> *and* sub-mechanism in `program.md` now has either a note or a lab result** — this time checked
> clause by clause rather than heading by heading. The durable lesson: *a coverage claim is only as
> fine-grained as the unit it was checked at.* The two remaining thin spots
> (`statistical-learning`'s gradient-boosting / small-neural-net clause, `range-variance`'s HAR-RV /
> vol-of-vol clause) are covered by analogy only, and this file argues against re-opening either;
...
**[Added 2026-09-08] The generating model for the artifact this lab has now measured twice, in
closed form — and it is not a lead-lag note, it is a measurement note that happens to live here.**
Lo–MacKinlay 1990 in *Journal of Econometrics* (Tier A, read in full; **not** the RFS paper of the
same year that this folder already holds) models non-synchronicity as random censoring: a security
fails to trade with probability `p`, and when it next trades its observed return is the sum of all
the virtual returns it missed. Every second moment then follows exactly. For **individual**
securities the induced own-autocorrelation is negative, proportional to `μ²`, and vanishes at zero
mean — small for daily data. For **portfolios** everything flips: `Corr[R°_κt, R°_κ,t+n] = p_κ^n`,
*positive*, equal to the non-trading probability itself and **independent of the mean**, so an
observed portfolio return series is an exact AR(1) in `p`. Diversification does not remove the
artifact; it converts a small negative one into a large positive one, and "if the non-trading
phenomenon is extant it will be most evident in portfolio returns." Portfolio variance is
*deflated* by `(1−p)/(1+p)`. The lagged cross-covariance carries `p_b^n` on the **lagging** side
only, so `γ_ab(n)/γ_ba(n) = (p_b/p_a)^n`: an "A leads B" pattern appears whenever `p_a < p_b` with
no diffusion mechanism present at all. Time aggregation is the cure and it is a *rate*, not a
threshold — the induced term is bounded in `q` while the signal term grows linearly, so relative
contamination falls like `1/q`. **The paper's own empirical verdict is negative and must be carried
with the mechanism**: for US common stocks the implied index autocorrelation is at most ~7.5%
using autocorrelation-based `p` estimates and under 2% using direct ones, several times smaller
than what is observed, and the authors conclude the evidence gives "little support for
nonsynchronous-trading as an important source of spurious correlation in the returns of common
stock". That is evidence about a single-session universe of actively traded names; whether it
transfers to fifteen trading sessions is not something the paper claims and this file must not
claim for it. `validation_overlap: false`, `published_post_2018: false`.
→ `notes/2026-09-08-nonsynchronous-trading-econometrics.md`

**Two exports from it that are worth more here than the mechanism.** First, an **overidentifying
specification test**: with `N_p` groups the `N_p²` lagged cross-covariances are constrained to
`N_p(N_p+1)/2` free parameters, and every chain ratio `γ_aκ1 γ_κ1κ2 … γ_κr b / γ_κ1a γ_κ2κ1 … γ_bκr`
must equal `(p_b/p_a)^n`. A pattern generated by non-synchronicity satisfies all of them; one
generated by information diffusion has no reason to. That is a sharper test than anything run in
this family so far and it costs no trial (#89). Second, a **corollary derived here from their
(2.24) and (2.27), not stated in the paper**: the observed contemporaneous correlation between two
groups is `√((1−p_a²)(1−p_b²))/(1−p_a p_b)`, equivalently `(ρ°)² = 1 − [(p_a−p_b)/(1−p_a p_b)]²` —
**exactly 1 when the two groups share a `p`, and attenuated only by the difference between them.**
That is precisely the statistic 2026-09-07 measured (same-region-minus-different-region mean
pairwise correlation, +0.144 daily against +0.030 weekly) and 2026-09-06 measured on regional
cross-serial residuals at ~4x. Two independent measurements of one artifact now have one generator,
...
**[Added 2026-09-08] The clause's last uncovered half — "stacking or meta-labelling over family
leads" — and it turns out to contain the structural reason every closure in this family was
inevitable.** Breiman's *Stacked Regressions* (Machine Learning 24, 49–64; Tier A, read in full) is
the estimator this lab has never used: every operator it has run either fixes the weights a priori
(the equal-weight mean) or uses none (max, intersection). Stacking **fits** them, under two
constraints that each repair a distinct failure. Weights must be fitted on **level-one data** — each
predictor's held-out predictions, generated by `J`-fold refitting — because weights fitted in
sample just load on whichever member overfits most. And the weights must be constrained
**non-negative**; Breiman reports ridge and its variants on the weights giving inconsistent
results, while non-negative least squares "almost always" beats the single best cross-validated
member. He is explicit that no general proof exists. `validation_overlap: false`,
`published_post_2018: false`. → `notes/2026-09-08-stacked-regressions-nonnegative-weights.md`

**The result that matters here is not the estimator but the bound it comes with, and it converts
four empirical closures into one theorem.** With `α_k ≥ 0` **and** `Σ α_k = 1`, the combination
satisfies `min_k v_k(x) ≤ Σ α_k v_k(x) ≤ max_k v_k(x)` pointwise — it is an **interpolating
predictor**, and stacking is the search for the best interpolator. **A fully-invested long-only
book satisfies both constraints by construction**: capital shares are non-negative and sum to one.
So "the gain is bounded by the components' disagreement" — recorded five times in `learnings.md`
as an empirical regularity of averaging — **is the simplex**, and no reweighting of finished
long-only books, learned or unlearned, can escape it. That covers HRP and every clustering
allocator (#86 by a second route), and it is the portfolio-side twin of Novy-Marx's linear
composite equivalence. **Breiman also names both exits**: drop the linearity (the max operator,
which the lab found empirically), or drop `Σα = 1` — his own stacks sum to 0.7–0.9 and 1.1–1.3 —
which for a book means **holding cash**. The repo's constraint is gross leverage ≤ 1.0, not = 1.0,
so that exit exists; its price is the subject of the next entry.

**Three of Breiman's own diagnostics predict, from outside, that a stack over this lab's legs will
collapse to selection.** (a) His gains come from **dissimilar** predictors; stacking ridge
regressions alone barely helped because adjacent members are near-identical functions. The lab's
eight leads correlate **0.68–0.98** with two pairs at 0.98 — that is the ridge pool. (b) The stack
is sparse: out of 40 candidates the average number receiving non-zero weight is **3.1** (subsets),
**1.6–2.3** (ridge), **3.4** (both pooled); out of ~50 subtrees, 6.3. Eight highly-correlated legs
should be expected to yield a stack of one or two. (c) **Theorem 1 is a necessary-and-sufficient
condition, not a heuristic**: the best single predictor is also the best stacked predictor iff
`R_kk ≤ R_ik` for all `i`, equivalently `ρ_ik ≥ σ_k/σ_i` on *residuals*. That is directly
computable and decides in advance whether any fitted-weight combination can beat the best single
leg — candidate #88, with the objective mismatch (squared error, not Sharpe) stated as its limit.

**[Added 2026-09-08] Meta-labelling, the clause's other word, and the theory that prices it
exactly.** The object: a **primary** model picks the side, a **secondary** classifier is trained on
"was the primary right" and its probability decides whether to take the bet and how big. Its
primaries are a trade book and two *Journal of Financial Data Science* articles carrying **one
recorded citation each**, none readable here — Tier C, and the construction was taken from the
authors' own reference implementation, not from any argument. **The structural point is that under
a long-only mandate the primary's side is always long, so the secondary has exactly one degree of
freedom — hold or hold less — which makes meta-labelling a market-timing overlay by identity, not
by analogy.** Merton (1981, *Journal of Business*, Tier A, read in full) then prices it with no
CAPM and no distributional assumption. With `p1 = P(filter off | bet would lose)` and
`p2 = P(filter on | bet would win)`: the **conditional** hit rates are the sufficient statistic and
unconditional accuracy is not; `p1 + p2 = 1` means the forecast is worth **exactly zero** to every
investor (Prop. III.2); `p1 + p2 > 1` is necessary and sufficient for positive value and the value
rises in the sum (Prop. III.3); a filter that is always on, or always off, has `p1 + p2 = 1` —
"like a stopped clock, such a forecast will sometimes be correct, but it never has any value"; and
the equilibrium value per period is `Δ · g · (p1 + p2 − 1)`, where `g` is the price of a one-period
put on the market struck at the riskless rate and `Δ` is the exposure swing between the two
states. Henriksson–Merton (Part II, Tier A, read in full) turn the null into an **exact
distribution-free test**: conditional on the two margins the count of correct "off" calls is
**hypergeometric**, independent of `p1` and `p2`, so the whole test is four counts.
`validation_overlap: false`, `published_post_2018: false` for the Merton pair; `true` for the
meta-labelling sources. → `notes/2026-09-08-meta-labeling-and-the-value-of-a-filter.md`

**Net: the family closes a fifth time, and this one closes with a theorem and a free test rather
than an arithmetic.** Stacking is the untried operator and its own author's diagnostics say it
collapses to selection on a pool like this one; meta-labelling is a timing overlay whose entire
value is one number the lab can test for nothing. Both are anti-candidates (#88's screen and #91's
test are the free parts). The connection between them is the point: the simplex bounds a
fully-invested long-only combination, the only linear escape is to hold cash, and holding cash is a
...
**[2026-09-08] The simplex is the bound, and this promotes an empirical regularity recorded five
times in this lab to a theorem.** `learnings.md` states in five places, each time as something
measured, that "the gain from aggregation is bounded by the components' disagreement": model
averaging across specifications, the five vintage axes, the eight-leg ensemble table that closed
`portfolio-learning`, #67's mean composite, and the tail-depth penalty. Breiman's interpolation
result is two lines of algebra and says why. If combination weights are **non-negative and sum to
one**, then for every input `min_k v_k ≤ Σ α_k v_k ≤ max_k v_k` pointwise — the combination is an
*interpolating predictor* and can never leave the range of its components. **A fully-invested
long-only book satisfies both constraints by construction.** Capital shares cannot be negative and,
if the book is fully invested, they sum to one. So the bound is not a property of averaging that
this lab happened to keep measuring; it is the geometry of the constraint set the engine imposes,
and it applies identically to equal weights, to risk-parity weights, to clustering allocators, to
learned combiners, and to any future weighting scheme anyone proposes. **The rule: before pricing
any proposal that reweights finished long-only books, note that it cannot beat its best component,
and ask instead what it is buying — variance reduction, turnover, capacity — because it is not
buying return.** Two exits exist and only two. Drop the **linearity**, which is what the lab's max
operator does and why it is the only aggregation result in the repo that escaped the bound (this is
also the general form of Novy-Marx's exact equivalence: linear composites *are* portfolios of their
legs, and the value of an aggregation operator is the value of its nonlinearity). Or drop
`Σ α = 1`, which for a book means **holding cash** — permitted here, since the constraint is gross
leverage ≤ 1.0 rather than = 1.0, and priced exactly by Merton at `Δ · g · (p1 + p2 − 1)`: skill
above chance, times the price of the put the timing pattern replicates, times the exposure swing.
Neither exit is free, but knowing there are exactly two is what stops the next weighting proposal
from being the fifth version of the first.
→ `notes/2026-09-08-stacked-regressions-nonnegative-weights.md`,
`notes/2026-09-08-meta-labeling-and-the-value-of-a-filter.md`,
`notes/2026-09-01-multi-signal-overfitting-critical-t.md`

**[2026-09-08] Before believing a second moment computed across this universe's fifteen trading
sessions, ask what it would look like with no mechanism in it — there is a closed form.**
Non-synchronous closes are a measurement problem with exact consequences, not a caveat.
Lo–MacKinlay's censoring model gives: portfolio autocorrelation `= p^n`, **positive and independent
of the mean**, so diversification amplifies rather than averages away; portfolio variance
**deflated** by `(1−p)/(1+p)`, so close-to-close volatility understates truth and unequally across
regions; lagged cross-covariance carrying `p^n` on the **lagging** side only, so `A leads B` appears
whenever `p_a < p_b` with no diffusion present; and observed contemporaneous cross-group correlation
attenuated by `√((1−p_a²)(1−p_b²))/(1−p_a p_b)`, which is **exactly 1 when two groups share a
staleness and falls only with the difference between them**. Aggregation is the cure and it is a
rate: the contamination falls like `1/q`, so one weekly reading is a partial correction and the
*sequence* over `q` is the informative object. **The rule, and it reaches past `lead-lag-spillover`
into every family**: any construction that ranks, clusters, or weights on a cross-instrument
correlation, covariance or close-to-close volatility across these sessions inherits a regional bias
with no information in it — including the `rho` this repo prices every blend on. Report the
statistic at `q = 1, 5, 10, 21` and let the trend say whether it is real. One counterweight worth
...
88. **[Added 2026-09-08] A free, necessary-and-sufficient screen that decides in advance whether
    *any* fitted-weight combination of the lab's legs can beat the best single one — and it is a
    theorem, not a calibration.** Breiman's Theorem 1: over a pool of predictors with residual
    cross-product matrix `R_ij = Σ_n (y_n − v_i(x_n))(y_n − v_j(x_n))`, the best single predictor
    `k` (minimal `R_kk`) is also the best **stacked** predictor **if and only if `R_kk ≤ R_ik` for
    every `i`** — equivalently `ρ_ik ≥ σ_k/σ_i`, a correlation of *residuals*, not of predictions
    and not of return series. Necessary and sufficient, proved from the Kuhn–Tucker conditions of
    the constrained quadratic program. **The measurement**: on the train split, take the leg pool's
    per-name signal values against whatever forward-return target the legs are predicting, build
    `R`, identify `k`, and check the inequality for every `i`. If it holds, selection is provably
    optimal and no stacking, learned combiner, clustering allocator or meta-labeller can improve on
    the incumbent leg — the family closes for a fifth time with a proof. If it fails for some `i`,
    that `i` is named: it is the one leg worth combining, which is strictly more information than
    "ensembles decline monotonically". **Two riders.** State the target and the horizon before
    computing, since both are researcher degrees of freedom. And carry the objective mismatch
    honestly: Breiman's algebra is for squared error, this repo maximises Sharpe, so the screen is
    a **kill switch and not a green light** — a failure of the condition licenses a candidate, it
    does not predict one. Free, train-only, no trial, no holdout look. Tier A, no validation
    overlap (no market data in the source at all).
    → `notes/2026-09-08-stacked-regressions-nonnegative-weights.md`

89. **[Added 2026-09-08] The overidentifying restriction test — a specification test on the lab's
    regional cross-covariance matrix that can *fail*, which is more than any screen this family has
    run.** Under Lo–MacKinlay's non-trading model the entire lagged cross-covariance structure of
    `N_p` groups is generated by `N_p` numbers, so the `N_p²` distinct covariances in `Γ_n` are
    constrained to `N_p(N_p+1)/2` free parameters, and for any chain of distinct groups
    `[γ_aκ1(n) γ_κ1κ2(n) … γ_κr b(n)] / [γ_κ1a(n) γ_κ2κ1(n) … γ_bκr(n)] = (p_b/p_a)^n`. **The
    measurement**: form the lab's regional (or ETF-versus-single-name) group return series on the
    train split, estimate `p_κ` two independent ways — as the `n`-th root of the group's own
    `n`-th order autocorrelation, and from the pairwise asymmetry ratio
    `p_b/p_a = [γ_ab(n)/γ_ba(n)]^{1/n}` — and check three things: that the two estimates agree,
    that the asymmetry ratio at `n = 2` is the square of the ratio at `n = 1`, and that the chain
    restrictions hold across groups. **Both answers are findings.** Consistency says the lab's
    regional pattern is measurement, which converts 2026-09-06's "we think this partition is
    measurement" into a statement that survived a test designed to break it. Violation says
    something else is there, and names which pairs violate it. **The rider is this repo's standing
    habit and is not optional**: `p` is estimated from an autocorrelation, and 2026-09-07 killed a
    `DELAY` statistic that reproduced its own null in level, dispersion *and* persistence — so
    simulate under a no-lag null first; the chain restrictions are themselves the natural placebo,
    since a null that satisfies them is measurement. **One honest limit**: the model's `p` is an
    i.i.d. censoring probability, while a time-zone offset is a deterministic lag, so a `p`
    estimated here means "the censoring parameter that reproduces these second moments", not a
    trading probability. Free, no trial. Tier A, no validation overlap.
    → `notes/2026-09-08-nonsynchronous-trading-econometrics.md`

90. **[Added 2026-09-08] A possible bias in the single number eight consecutive sessions of blend
    declines rest on, with a four-line measurement that settles it.** The required-gain table is
    read off `rho` between a candidate's and the champion's return series, and the paired standard
    error `√(1−rho)` is the resolution floor for every promotion decision this lab makes. Those
    series are computed from daily USD closes across fifteen trading sessions. The corollary
    derived in the note — observed contemporaneous correlation
    `= √((1−p_a²)(1−p_b²))/(1−p_a p_b)`, i.e. `(ρ°)² = 1 − [(p_a−p_b)/(1−p_a p_b)]²` — says the
    attenuation is **zero when two books have the same effective staleness and grows with the
    difference between them**. The seated `liquidity-volume` lead is a *region-relative* `ILLIQ`
    construction, i.e. regionally tilted relative to the seat by design, which is exactly the case
    where the difference is not zero. **The measurement**: recompute champion-versus-leg `rho` at
    `q = 1, 5, 10, 21` and report the sequence, not one horizon. Flat means the two books share an
    effective `p` and the daily number is clean; rising in `q` means it is not, and the model says
    the contamination falls like `1/q`, so a single weekly reading is a partial correction rather
    than a clean one. **Direction of the error, stated so nobody over-reads a pass**: attenuation
    biases `rho` *down*, which inflates the paired SE and pushes the required-gain table *upward* —
    the lab has been conservative, not permissive. This is therefore a correction that could only
    ever make a blend look better, and it should be run precisely because that is the direction
    that would change a decision. Free, no trial. Tier A, no validation overlap.
    → `notes/2026-09-08-nonsynchronous-trading-econometrics.md`

91. **[Added 2026-09-08] A four-count exact test that prices any "when to hold, when to sit out"
    overlay before it is built — and an anti-candidate for the meta-labelling construction that
    would otherwise be the obvious next `portfolio-learning` move.** Under a long-only mandate a
    meta-labelling second stage has exactly one degree of freedom, hold or hold less, so it is a
    market-timing overlay by identity. Merton's theory then applies exactly, with no CAPM and no
    distributional assumption: with `p1 = P(off | the bet would have lost)` and
    `p2 = P(on | the bet would have won)`, the forecast is worth **exactly zero** iff
    `p1 + p2 = 1`, positive value requires `p1 + p2 > 1`, and unconditional accuracy is **not** a
    sufficient statistic. **The measurement**: reduce the proposed gate to a binary decision per
    non-overlapping rebalance date on the train split, count `N1`, `N2` and `n`, and run the
    Henriksson–Merton hypergeometric test — `P(n1 = x) = C(N1,x)C(N2,n−x)/C(N1+N2,n)`, exact,
    distribution-free, and independent of `p1` and `p2`, so neither needs estimating. If
    `p1 + p2` is not reliably away from 1, the gate is worthless *whatever its model class*, and
    no feature engineering or probability calibration changes it. **The trap this screen exists to
    catch** is the stopped clock: a filter that is off on 5% of dates has a high raw hit rate and
    `p1 + p2` barely above 1. The hypergeometric conditions on `n`, which is exactly why it must be
    used instead of an accuracy or an IC — the same discipline as 2026-09-06's "pin breadth by
    construction", reached from a different direction. **Also read the sizing term**: equilibrium
    value is `Δ · g · (p1 + p2 − 1)`, so a gate with real skill still pays nothing if the exposure
    swing `Δ` is small, while a large `Δ` in a long-only book means large swings to cash whose
    turnover Merton's frictionless model does not charge and this repo does. **Anti-candidate
    status**: do not spend a trial on a meta-labelled construction. Its own primaries are a trade
    book and two practitioner articles with one recorded citation each (Tier C); `program.md`'s
    closure instructs against a further operator over these legs; and `learnings.md`'s "blending
    beats switching" is the measured instance of what the theory predicts. The screen is the
    deliverable, and it would have been the right first question for the regime-switching and
    turbulence-gated candidates the lab already ran and refuted. Free, train-only, no trial.
...
- **[2026-09-08] Read this first: the 2026-09-07 nightly ran and declined everything, which is a
  ...
  six proposed candidates against the idea rather than against a written file — the right call by
  its own stated bar, and the second time in this lab's history it has been made. **Of the
  pre-2026-09-08 list, only #82 (`J*` bandwidth, free, with its placebo rider) is still unrun.**
  #85, #79 and #86 are all decided, all negatively. The nightly's own message is the one to carry:
  when a session declines every constructible idea, **the bottleneck is not this folder's supply of
  proposals**, and the correct response is not to write more of them. That is why tonight's four
  candidates are all *screens* — three of them tests that can fail — and only one of them
  (#88's, on a failure) could ever license building anything.
- **[2026-09-08] What should aim the next session, in order.**
  - **#88 first, and it is the one that could close a family with a proof rather than an
    arithmetic.** Breiman's Theorem 1 is necessary *and* sufficient, computable on the train split,
    and it decides whether any fitted-weight combination of the leg pool can beat the best single
    leg. Carry its stated limit: squared error, not Sharpe, so it kills but does not license.
  - **Then #89 and #90 as one reading, not two.** They share an estimator. #89 estimates each
    group's effective staleness two independent ways and tests the overidentifying restrictions;
    #90 applies the same attenuation formula to the champion-versus-leg `rho` at `q = 1, 5, 10, 21`.
    Run together they cost one afternoon of train-split computation and they audit, respectively,
    the family the lab closed on measurement and the number every promotion decision is priced on.
    **#90 is the one to run if only one is run**, because it is the only item on this list that
    could move a decision the lab has made eight times.
  - **#91 is a screen attached to an anti-candidate.** Do not build a meta-labelled construction;
    do run the four-count hypergeometric test on any future gating, timing or state-dependence
    proposal *before* it is written, exactly as #85's breadth rider should have been run before its
    excess was read.
  - **#82 is still unrun and still free.** It has now been carried for three sessions.
- **[2026-09-08] The sub-mechanism gap is now genuinely closed, and here is what that means for the
  next focus.** 2026-09-07 established the rule "when the family list looks exhausted, re-read
  `program.md`'s sub-mechanism clauses rather than the family headings" and found two gaps
  (cointegration, HRP). Tonight closed the last one it named — `portfolio-learning`'s stacking and
  meta-labelling clause. **Every named sub-mechanism in `program.md` now has either a note or a lab
  result.** The two remaining thin spots are both ones this file has already argued against
  re-opening: `statistical-learning`'s gradient-boosting and small-neural-net clause (covered only
  through the comparative-method literature, and the standing conclusion is that a 140-name universe
  cannot support a heavy learner), and `range-variance`'s HAR-RV and vol-of-vol clause (a family
  closed fourteen times over, now with **two independent passing robustness tests on the artifact**
  — the lab's 2026-09-07 phase screen ranked Garman-Klass volatility the second most phase-stable
  score on the board, corroborating 2026-09-06's monotonicity result, and both are passes *on the
  identified survivorship artifact*). **So the next session should not look for an uncovered
  mechanism; there is not one.** The honest options are (a) deepen the *measurement* vein this
  session opened, which is the only one that has produced a correction to a live decision, or (b)
  take the lab's own standing recommendation seriously: `range-variance` is "unreachable on this
  universe rather than unexplored", the cold-family rule has gone unsatisfied for seven sessions,
  and amending or retiring it is an edit to a frozen file that **needs a human**. This folder cannot
  make that edit and should stop generating proposals that route around it.
- **[2026-09-08] An embargo boundary tested and held, and a new one drawn.** Lo–MacKinlay report
  that the non-trading model implies a weekly index autocorrelation of at most ~7.5%, and under 2%
  on direct estimates, against a materially larger observed figure. Those are **model-implied
  magnitudes and a property of an estimator**, recorded because the paper's own conclusion — that
  non-synchronicity does *not* explain US index autocorrelation — depends on them and would be
  dishonest to omit; the observed figure they are compared against is described qualitatively and
  its sample dates live only in the note's frontmatter. The new line: **a magnitude that a model
  implies, or that an estimator can produce under its own null, is decision-relevant here and is
  recorded; a magnitude that a market realised in a window is not.** Merton's Part I contains a
  numerical illustration of a hypothetical fund's returns and **none of it was carried across**,
  because a worked example's numbers are neither of those things.
- **[2026-09-08] Access notes for the recipe, including one new channel and one confirmed
  blockage.** **The Internet Archive's `_djvu.txt` is the session's find and belongs on the
  README's list**: `archive.org/metadata/<identifier>` reveals whether a scanned item has OCR text,
  and for both Merton working papers it did — served instantly, while **MIT DSpace returned HTTP
  429 to every request including five retries with exponential backoff**, which is a rate limit
  rather than an egress block and should be reported as such. Also new: **`pymupdf`** installs into
  a scratchpad `pylibs` alongside `pypdf` and renders a text-layerless scan to PNG directly,
  removing the missing-`pdftoppm` dependency the README documents. Confirmed working first try: the
  **UC Berkeley statistics tech-report archive** (which serves the typeset published article, not
  the draft, despite the URL) and **NBER's `system/files/working_papers/` path**. Confirmed
  refusing an automated client: **`pm-research.com`**, which redirects into an OpenID authorization
  flow rather than returning a Cloudflare challenge — a third distinct refusal mode after the 403
  challenge and the metered budget.
