...
## 2026-08-13T01:11:49+00:00 — mom_residual_raw_zscore_trim — **REJECT**
- Candidate: `strategies/candidates/mom_residual_raw_zscore_trim.py` (family: cross-sectional momentum, trial #33)
- Hypothesis: Scoring the same buffered magnitude-weighted basket by cumulative market-model residual return (beta removed) WITHOUT dividing by residual volatility retains trial #32's best-ever validation maxDD (-24.5%) while restoring the annualized return that the vol normalization destroyed (+16.0% vs +27.0%), lifting validation Sharpe above 1.07 net of 15 bps costs, because the beta removal and the risk normalization are separable and only the latter tilts capital away from the high-conviction compounders this repo has repeatedly found to carry the return.
- Verdict: REJECT — deflated sharpe prob 0.9074 < 0.95 (bar set by 33 total trials)
- Train: sharpe +0.96, ann_ret +19.3%, maxDD -64.1%, turnover 4.3x
- Validation: sharpe +1.02, ann_ret +23.5%, maxDD -29.9%, turnover 7.4x
- Deflated Sharpe prob: 0.9074 (bar from 33 trials)
- Champion validation sharpe at the time: +0.86
- Lesson: **The disentangling worked, but the attribution is the reverse of what this repo's learnings predicted — the residual-vol denominator is a risk lever, not a return drag, and beta removal on its own buys nothing.** Dropping the vol normalization did restore return as expected (+16.0% -> +23.5%, Sharpe 0.96 -> 1.02), confirming that half (b) was what suppressed the return. But maxDD reverted almost exactly to the raw-return version's level (-24.5% -> -29.9%, vs -30.3%), which means trial #32's best-ever drawdown came from dividing by idiosyncratic vol, *not* from removing market beta. Comparing this trial against the raw-return baseline (`mom_zscore_narrow_daily_volspike_trim`: Sharpe 1.07, ann_ret +27.0%, maxDD -30.3%, turnover 7.3x) isolates beta removal as a standalone change: it is mildly negative on every axis (1.02 / +23.5% / -29.9% / 7.4x) and dramatically worse on train drawdown (-64.1% vs -54.7%), because scoring on beta-stripped returns without any risk normalization promotes high-idiosyncratic-vol names that raw total-return ranking would not have reached. Two conclusions: (1) residual/beta-neutralized momentum is refuted as a signal upgrade on this universe — the market-beta component of trailing returns is not noise to be removed here, it is part of what the momentum signal is profitably picking up, consistent with the earlier sector-neutralization refutation (both are 'neutralize a common factor out of the score' ideas and both cost Sharpe); (2) idiosyncratic-vol normalization is worth remembering as the repo's most effective *drawdown* mechanism found so far (-24.5%, better than the daily vol-spike trim's -30.3%), but at a return cost far too large to be a Sharpe upgrade on its own.

## 2026-08-13T01:14:26+00:00 — mom_infodiscreteness_zscore_trim — **REJECT**
- Candidate: `strategies/candidates/mom_infodiscreteness_zscore_trim.py` (family: cross-sectional momentum, trial #34)
- Hypothesis: Adding a third equally-weighted composite term for information continuity (-ID, where ID = sign(formation return) * (%neg - %pos) of daily returns over the 252d formation window, skipping the most recent month) to the buffered magnitude-weighted momentum basket raises validation Sharpe above the 1.07 of the otherwise identical two-term version, net of 15 bps costs, because momentum built from gradual same-signed drift continues more reliably than momentum built from a few large jumps that the market has already priced.
- Verdict: REJECT — deflated sharpe prob 0.8915 < 0.95 (bar set by 34 total trials)
- Train: sharpe +0.96, ann_ret +18.6%, maxDD -57.7%, turnover 4.8x
- Validation: sharpe +0.99, ann_ret +25.5%, maxDD -35.5%, turnover 7.2x
- Deflated Sharpe prob: 0.8915 (bar from 34 trials)
- Champion validation sharpe at the time: +0.86
- Lesson: **Refuted, and it fails differently from the two residual trials — information continuity is not a risk lever either, it is simply dilution.** Validation Sharpe fell to 0.99 (vs 1.07 for the identical two-term version), with return down (+25.5% vs +27.0%) and maxDD *worse* (-35.5% vs -30.3%), at essentially unchanged turnover (7.2x vs 7.3x). The pre-registered alternative outcome — that continuity would behave like trial #32's vol denominator, buying drawdown at the cost of return — did not happen: it gave up ground on both axes at once. The mechanism is most likely that the third term carries no independent edge on this universe while claiming a full one-third of the composite's weight, so it displaces the return-magnitude signal that four prior trials established as the actual driver (the same dilution arithmetic as the ETF-blend learning: a term with no edge costs Sharpe in proportion to the capital it steers). This universe is also 140 global large-caps rather than the broad US cross-section the frog-in-the-pan effect was documented on, where the inattention channel it relies on is weakest — these are the most-covered names in the world. Two negatives now point the same way: after 52-week-high proximity (trial #31), residual momentum (#32, #33) and information discreteness (#34), no alternative or supplementary price-based scoring dimension has beaten plain composite return magnitude on this universe. Treat the 'find a better score' axis as heavily explored and low-yield; the untested axes left are portfolio construction and rebalance mechanics, not signal definition.

## 2026-08-13T01:17:55+00:00 — mom_zscore_tranched_trim — **REJECT**
- Candidate: `strategies/candidates/mom_zscore_tranched_trim.py` (family: cross-sectional momentum, trial #35)
- Hypothesis: Splitting the buffered magnitude-weighted momentum basket into four weekly-staggered tranches, each rebalanced every fourth week with its own buffer state and averaged into one portfolio, raises validation Sharpe above the 1.07 of the identical single-date month-end version and reduces its -30.3% maxDD, net of 15 bps costs, because averaging over formation dates removes rebalance timing luck rather than changing the signal.
- Verdict: REJECT — deflated sharpe prob 0.9148 < 0.95 (bar set by 35 total trials)
- Train: sharpe +0.97, ann_ret +19.4%, maxDD -50.9%, turnover 4.6x
- Validation: sharpe +1.05, ann_ret +25.9%, maxDD -29.6%, turnover 7.8x
- Deflated Sharpe prob: 0.9148 (bar from 35 trials)
- Champion validation sharpe at the time: +0.86
- Lesson: **A clean null, and a genuinely reassuring one: the repo's best challenger is not a rebalance-timing-luck artifact.** Averaging four weekly-staggered formation dates left validation Sharpe at 1.05 vs the single-date version's 1.07 — inside noise — while maxDD improved slightly on *both* splits (validation -30.3% -> -29.6%, train -54.7% -> -50.9%) at a modest turnover cost (7.3x -> 7.8x, from a quarter of the book reforming weekly rather than all of it monthly). Formation-date dispersion is therefore small at monthly frequency on this universe, which matters beyond this trial: every candidate in the repo's history has been evaluated on one arbitrary month-end formation date, and this is the first evidence that the resulting numbers are not materially a draw from a wide timing-luck distribution. The negative read is that timing diversification is not a free Sharpe upgrade here — unlike membership buffering (trial #17), which attacked a real inefficiency and delivered — because there was no large inefficiency to remove in the first place. Do not retry with a different tranche count or grid frequency; that would be a knob sweep on a mechanism just shown to be near-neutral. The interesting residue is that the drawdown improvement showed up on both splits rather than one, which is the signature of a real if small effect rather than noise.

## 2026-08-13T01:23:33+00:00 — mom_zscore_weightband_trim — **REJECT**
- Candidate: `strategies/candidates/mom_zscore_weightband_trim.py` (family: cross-sectional momentum, trial #36)
- Hypothesis: Adding a 2-percentage-point no-trade band on the magnitude weights of already-held names — leaving a position at its drifted weight unless its new target differs by more than the band, with membership and the daily vol-spike trim unchanged — cuts turnover below trial #28's 7.3x and raises validation Sharpe above its 1.07 net of 15 bps costs, because most month-to-month movement in the composite z-score of a name that never left the basket is jitter rather than information, and trading on it is a pure cost.
- Verdict: REJECT — deflated sharpe prob 0.9167 < 0.95 (bar set by 36 total trials)
- Train: sharpe +0.97, ann_ret +19.6%, maxDD -53.8%, turnover 3.9x
- Validation: sharpe +1.06, ann_ret +26.8%, maxDD -30.6%, turnover 6.8x
- Deflated Sharpe prob: 0.9167 (bar from 36 trials)
- Champion validation sharpe at the time: +0.86
