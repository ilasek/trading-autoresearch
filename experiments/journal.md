...
## 2026-08-14T01:10:47+00:00 — mom_zscore_continuous_voltarget — **REJECT**
- Candidate: `strategies/candidates/mom_zscore_continuous_voltarget.py` (family: cross-sectional momentum, trial #32)
- Hypothesis: Replacing the binary vol-spike exposure trim (21d/252d realized-vol ratio > 1.6 -> 0.6x) with a continuous proportional scalar min(1, sigma_252/sigma_21) on the identical narrow hold-25/enter-15 buffered composite-z-score basket raises validation Sharpe above the binary version's 1.07, net of 15 bps costs, because a proportional response uses the magnitude of the vol expansion instead of discarding it at a threshold.
- Verdict: REJECT — deflated sharpe prob 0.8991 < 0.95 (bar set by 32 total trials)
- Train: sharpe +0.96, ann_ret +18.2%, maxDD -54.2%, turnover 5.4x
- Validation: sharpe +1.01, ann_ret +23.9%, maxDD -30.4%, turnover 8.3x
- Deflated Sharpe prob: 0.8991 (bar from 32 trials)
- Champion validation sharpe at the time: +0.86
- Lesson: **Refuted, and informatively so — the binary trim's large inaction region is a feature, not a crudeness to be smoothed away.** Against the identical-in-every-other-respect binary version (trial #28), the continuous scalar delivered *less* return (23.9% vs 27.0%) and *less* vol (24.2% vs 25.4%) for a net Sharpe loss (1.01 vs 1.07), with maxDD essentially unchanged (-30.4% vs -30.3%) and turnover up (8.3x vs 7.3x). The decomposition is unambiguous: `min(1, sigma_252/sigma_21)` is below 1.0 on roughly half of all days, since 21d vol exceeds 252d vol about as often as not in a trending equity basket, so the overlay behaves as a *persistent partial capital drag in normal markets* while adding nothing during actual crashes — the binary rule already captures the crash days, which is why maxDD is identical. Meanwhile the proportional scalar nudges the entire book every single day at 15 bps per side, adding ~1x annual turnover for that privilege. So the Moreira-Muir proportional-response result does not transfer here, and the reason is specific and diagnosable: their construction earns most of its edge by levering *up* in calm regimes (2-3x gross), which this engine's `max_leverage = 1.0` forbids. Stripped of the lever-up half, proportional vol-management is all drag and no boost, whereas a threshold rule spends its de-risking budget only where the payoff is. **General lesson for overlay design under a hard leverage cap: prefer overlays that are inert by default and act rarely, over ones that respond continuously — the inaction region is what protects the base strategy's compounding.** This is a functional-form finding, not a threshold-tuning one; it does not license a sweep of trim thresholds or scale factors (still closed), but it does suggest future overlays be judged on what fraction of days they are non-neutral.

## 2026-08-14T01:14:11+00:00 — mom_residual_zscore_trim — **REJECT**
- Candidate: `strategies/candidates/mom_residual_zscore_trim.py` (family: cross-sectional momentum, trial #33)
- Hypothesis: Ranking and magnitude-weighting the buffered hold-25/enter-15 basket by a composite z-score of market-beta-residual cumulative return (252d and 126d windows, skip 21d), instead of total return, raises validation Sharpe above the total-return version's 1.07 net of 15 bps costs, because residualizing removes the unintended market-beta bet that total-return momentum accumulates and that drives its crashes.
- Verdict: REJECT — deflated sharpe prob 0.9222 < 0.95 (bar set by 33 total trials)
- Train: sharpe +0.93, ann_ret +18.3%, maxDD -65.2%, turnover 4.5x
- Validation: sharpe +1.07, ann_ret +24.6%, maxDD -28.8%, turnover 7.6x
- Deflated Sharpe prob: 0.9222 (bar from 33 trials)
- Champion validation sharpe at the time: +0.86
- Lesson: **A dead heat on the objective, and the first alternative signal in this repo that is neither an improvement nor a refutation — which makes it interesting for a different reason than the one it was proposed for.** Validation Sharpe 1.068 vs the total-return version's 1.066: statistically indistinguishable. But the two are not the same portfolio wearing different clothes — residualization moved return and risk *together*: ann_ret 24.6% vs 27.0%, ann_vol 23.1% vs 25.4%, validation maxDD -28.8% vs -30.3%, Sortino 1.455 vs 1.415. So the beta-stripping did exactly what Blitz et al. predict on the risk side (a genuinely tamer return stream, better on every downside measure) and gave back precisely as much return as it saved in risk. **The crash-mitigation half of the claim is contradicted outright on this universe, though: train-period maxDD got much worse, -65.2% vs -54.7%.** The likely cause is factor mis-specification rather than a failure of the idea — the market proxy here is the equal-weighted cross-sectional mean of a universe that is roughly a third bond/gold/commodity/sector ETFs, so it is not a clean equity-market factor; residualizing against it subtracts a diluted, partly-irrelevant common component while leaving much of the real equity beta in place, and the betas themselves are noisy when estimated on ~140 heterogeneous series. The 2008-09 window is where that mis-specification would hurt most, which matches where the damage shows up. Two things follow. (1) Do not treat residual momentum as refuted — it is a same-quality signal, and a cleaner factor definition (e.g. residualizing stocks against an equity-only proxy and leaving ETFs unresidualized) is a distinct, still-open variant. (2) More promising and immediately actionable: two signals of *equal* Sharpe that arrive there by visibly different routes are the ideal ingredients for a score-level ensemble, and unlike every previously-tested blend in this repo there is no dilution tax to pay, because neither leg is the weaker one. That is the natural next trial, and it is a different mechanism from the refuted capital-blends (which split capital between two baskets); this would average the *scores* and still hold a single concentrated basket.

## 2026-08-14T01:18:40+00:00 — mom_zscore_notrade_band — **REJECT**
- Candidate: `strategies/candidates/mom_zscore_notrade_band.py` (family: cross-sectional momentum, trial #34)
- Hypothesis: Leaving a retained name's position untouched when its newly-computed target weight is within 25% of the weight already held — a no-trade band on size, on top of the existing hold-25/enter-15 band on membership — raises validation Sharpe above 1.07 net of 15 bps costs, because most of the basket's 7.3x annual turnover is re-sizing names it keeps anyway, and paying 30 bps round-trip to chase second-order wiggles in a noisy score costs more than the tracking error it avoids.
- Verdict: REJECT — deflated sharpe prob 0.9231 < 0.95 (bar set by 34 total trials)
- Train: sharpe +0.97, ann_ret +19.6%, maxDD -54.1%, turnover 4.1x
- Validation: sharpe +1.08, ann_ret +27.5%, maxDD -30.3%, turnover 7.0x
- Deflated Sharpe prob: 0.9231 (bar from 34 trials)
- Champion validation sharpe at the time: +0.86
- Lesson: **Directionally right and the new best challenger (val Sharpe 1.080 vs 1.066, turnover 7.0x vs 7.3x, identical maxDD -30.3%) — but the gain is far smaller than the premise predicted, and finding out why produced the more valuable result.** The band was expected to remove a large slice of the 4.7x/yr of re-sizing turnover; it removed 0.35x. A post-hoc decomposition of the *effective weight matrix* (a diagnostic, not a second backtest) explains the shortfall precisely. Of trial #28's 7.30x annual validation turnover, entries/exits account for 2.63x (36%) and re-sizing names that stay in the basket accounts for 4.67x (64%) — so the premise's first half was right, most turnover really is re-sizing. But only 7% of that re-sizing fell inside a 25%-of-position band, meaning **the monthly re-sizing moves are overwhelmingly large ones, not the small wiggles a no-trade band is designed to suppress.** The cause is structural and sits in the weighting formula itself: weights come from `raw = composite - composite.min() + 0.05` normalized, so the entire weight vector is anchored to *the weakest current member's score*. Whenever the marginal name changes — which the hold-25/enter-15 buffer guarantees happens often — the anchor moves and every single weight in the basket is rescaled, even for names whose own score and rank did not move at all. The cross-sectional re-standardization of the z-scores each month compounds it. So a large fraction of this basket's turnover is a pure artifact of a membership-dependent normalization, unrelated to any change in what the signal is saying. **That reframes the cost problem: the target is not to trade less toward a good target, it is to stop the target from moving for no reason.** The no-trade band is worth keeping (it is free, +0.015 Sharpe and slightly lower turnover), but the next trial should attack the anchor.

## 2026-08-14T01:21:25+00:00 — mom_zscore_fixed_anchor — **REJECT**
- Candidate: `strategies/candidates/mom_zscore_fixed_anchor.py` (family: cross-sectional momentum, trial #35)
- Hypothesis: Sizing positions by the composite z-score floored at a fixed constant, instead of by the score minus the weakest current member's score, raises validation Sharpe above the 1.080 of the otherwise-identical trial #34 basket and lowers its 7.0x turnover, because anchoring weights to the marginal member rescales the whole portfolio whenever that member is swapped — generating large re-sizing trades that carry no signal.
- Verdict: REJECT — deflated sharpe prob 0.9233 < 0.95 (bar set by 35 total trials)
- Train: sharpe +0.92, ann_ret +18.7%, maxDD -55.7%, turnover 4.5x
- Validation: sharpe +1.08, ann_ret +25.4%, maxDD -28.2%, turnover 5.8x
- Deflated Sharpe prob: 0.9233 (bar from 35 trials)
- Champion validation sharpe at the time: +0.86
- Lesson: **The mechanism hypothesis is confirmed outright and the cost saving is large; the Sharpe barely moved because a second, unanticipated effect ran the other way — and that second effect is the actionable finding.** New best challenger on Sharpe (1.085 vs 1.080), and by some distance the best risk profile in the repo's history: maxDD -28.2% (vs -30.3%) and turnover 5.75x (vs 6.97x), with ann_vol down to 23.4% from 25.6%. The weight-matrix decomposition confirms the anchor artifact exactly as predicted: **re-sizing turnover fell from 4.32x to 2.30x per year, very nearly halved**, which is direct evidence that roughly half the re-sizing trades in every magnitude-weighted candidate since trial #20 were caused by the weight vector being anchored to the swapped-out marginal member rather than by any change in signal. That artifact is now identified and fixed, and the fix is free. What was *not* predicted: the fixed anchor made the portfolio substantially **less** concentrated, not more — mean top weight 0.143 vs 0.194, HHI 0.0705 vs 0.0989. The write-up's reasoning was backwards, and the reason is instructive. `composite - composite[held].min()` subtracts the weakest *held* member's score, and thanks to the hold-25 buffer that member has usually decayed to a **negative** composite; subtracting a negative number inflates the spread between the top name and the bottom name far more than the fixed floor does. So the min-shift was never a compression device, it was a hidden concentration amplifier whose strength varied month to month with how badly the worst held name had decayed. That means the repo's whole equal -> rank -> magnitude escalation ladder (0.90 -> 0.93 -> 0.98 -> 1.03) was riding on an *unstable* concentration mechanism, which is very likely also why validation maxDD widened at every rung. Here the two effects roughly cancelled: de-concentration cost Sharpe, lower turnover and vol paid it back, net +0.005. **The clear next step is to restore the old concentration level through a stable, membership-independent transform — this is not the "further escalation" prior learnings warned against, since the target is to match the concentration already tested, only with mechanics that do not manufacture turnover or drift month to month.**

## 2026-08-14T01:24:20+00:00 — mom_zscore_convex_anchor — **REJECT**
- Candidate: `strategies/candidates/mom_zscore_convex_anchor.py` (family: cross-sectional momentum, trial #36)
- Hypothesis: Weighting by the composite z-score raised to the power 1.5 off a fixed floor — restoring the concentration level of the old membership-anchored formula (validation-era HHI 0.097 vs 0.105) without its instability — raises validation Sharpe above trial #35's 1.085 while keeping its 5.75x turnover and -28.2% maxDD, because concentration and anchor stability are independent levers and the repo's Sharpe gains from concentration were real.
- Verdict: REJECT — deflated sharpe prob 0.918 < 0.95 (bar set by 36 total trials)
- Train: sharpe +0.92, ann_ret +19.8%, maxDD -56.8%, turnover 5.0x
- Validation: sharpe +1.07, ann_ret +26.7%, maxDD -30.1%, turnover 6.3x
- Deflated Sharpe prob: 0.918 (bar from 36 trials)
- Champion validation sharpe at the time: +0.86
