...
## 2026-09-17T23:14:24+00:00 — pl_factor_momentum_timed — **SCOUT**
- Candidate: `strategies/candidates/pl_factor_momentum_timed.py` (family: portfolio-learning, track: scout, trial #92)
- Hypothesis: A long-only book that equal-weights twelve characteristic sleeves (both legs of trend, reversal, Amihud illiquidity, Garman-Klass range volatility, volume shock and dollar-volume rank) but holds only those whose OWN trailing 12-month return, demeaned against the equal-weighted universe, was positive at formation scores a validation Sharpe of 0.75 (range 0.45-1.10) and, decisively, scores ABOVE its otherwise bit-identical always-on partner `pl_factor_momentum_untimed`, because a characteristic sort is positively autocorrelated in its own past return: on train the timed book beats the untimed one by +1.55%/yr (t = +2.25, Sharpe 1.180 vs 1.109) with a hash placebo at the 100th percentile of 40 draws, the sign surviving all six leave-one-characteristic-out drops, and the pre-registered un-demeaned control reading BELOW the untimed book as the mechanism requires.
- Verdict: SCOUT — scouted family 'portfolio-learning': validation sharpe 0.755 <= the family's best 1.008 (DSR 0.7799, 92 trials, 26 effective after clustering at rho 0.95)
- Train: sharpe +0.74, ann_ret +8.3%, maxDD -44.6%, turnover 2.6x
- Validation: sharpe +0.76, ann_ret +12.3%, maxDD -30.3%, turnover 6.2x
- Deflated Sharpe prob: 0.7799 (bar from 92 trials, 26 effective)
- Scout track: family best before this trial +1.01; the champion was not compared and the holdout was not read
- Lesson: **The pre-registered level landed almost exactly (0.755 against 0.75, band 0.45-1.10) and the pre-registered CONTRAST landed backwards** — the timed book scores 0.112 BELOW its always-on partner where the train screen put it +0.071 above. Cost is ruled out for once: the timing rule costs 1.5x of extra annual turnover, ~0.23%/yr at this repo's constant, against a 2.10pp gap in annual return, so **cost explains 11% of it** and the other 89% is the selection. The deficit lands in 2021/2022/2023 (+14.2/-17.1/+27.2 against the control's +20.9/-13.3/+30.1) and 2018-2020 are near-ties — the rotation-year signature this repo has recorded for every construction that responds slowly to a leadership change, arriving this time on a rule that switches rather than one that blurs. `rho` to the control 0.9843, closed-form paired SE 0.071 (block-bootstrap 0.065), **t = -1.58 to -1.74, P(timed better) = 0.029**: directional, not resolvable at |t| = 2, and reported as both.

## 2026-09-17T23:15:30+00:00 — pl_factor_momentum_untimed — **SCOUT**
- Candidate: `strategies/candidates/pl_factor_momentum_untimed.py` (family: portfolio-learning, track: scout, trial #93)
- Hypothesis: The always-on control for `pl_factor_momentum_timed` — the same twelve characteristic sleeves (both legs of trend, reversal, Amihud illiquidity, Garman-Klass range volatility, volume shock and dollar-volume rank), equal weighted, every sleeve held every month — scores a validation Sharpe of 0.75 (range 0.45-1.10) and BELOW its timed partner, because a characteristic sort is positively autocorrelated in its own past return and dropping the sleeves whose own demeaned trailing 12-month return was negative is worth +1.55%/yr on train (t = +2.25) against a ~0.36%/yr cost difference; if this control wins instead, the published reexamination's result that timing does not beat buy-and-hold of the same factors is reproduced on this universe and `learnings.md`'s 'blending beats switching' extends from external state variables to a component's own past return.
- Verdict: SCOUT — scouted family 'portfolio-learning': validation sharpe 0.867 <= the family's best 1.008 (DSR 0.8569, 93 trials, 24 effective after clustering at rho 0.95)
- Train: sharpe +0.70, ann_ret +7.8%, maxDD -48.5%, turnover 1.6x
- Validation: sharpe +0.87, ann_ret +14.4%, maxDD -30.5%, turnover 4.7x
- Deflated Sharpe prob: 0.8569 (bar from 93 trials, 24 effective)
- Scout track: family best before this trial +1.01; the champion was not compared and the holdout was not read
