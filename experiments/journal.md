...
- No engine issues encountered this session.
## 2026-08-12T01:10:39+00:00 — mom_residual_zscore_daily_trim — **REJECT**
- Candidate: `strategies/candidates/mom_residual_zscore_daily_trim.py` (family: cross-sectional momentum, trial #32)
- Hypothesis: Ranking and magnitude-weighting the buffered basket by a composite z-score of cumulative *residual* return (total return minus beta-times-market return, beta estimated over a trailing 756d window) instead of cumulative total return raises validation Sharpe above the 1.07 of the otherwise-identical total-return version, net of 15 bps costs, because removing the market-beta component of each name's trailing move leaves a higher signal-to-noise, less crash-prone stock-specific momentum signal.
- Verdict: REJECT — deflated sharpe prob 0.8647 < 0.95 (bar set by 32 total trials)
- Train: sharpe +0.96, ann_ret +17.8%, maxDD -62.7%, turnover 4.0x
- Validation: sharpe +0.93, ann_ret +22.3%, maxDD -33.0%, turnover 7.2x
- Deflated Sharpe prob: 0.8647 (bar from 32 trials)
- Champion validation sharpe at the time: +0.86
- Lesson: **Refuted — market-neutralising the momentum signal removes real return on this universe without buying back a commensurate amount of risk.** Against the otherwise-identical total-return version (`mom_zscore_narrow_daily_volspike_trim`, trial #28), validation ann_return fell from 27.0% to 22.3% while ann_vol barely moved (25.4% -> 25.0%), so Sharpe dropped 1.07 -> 0.93 and maxDD *worsened* (-30.3% -> -33.0%); train maxDD deteriorated far more (-54.7% -> -62.7%). The trade the residualisation makes is therefore a bad one here: it strips out the beta-loading component of each name's trailing move, but on this survivorship-biased universe of today's global mega-caps that beta loading is a large part of where the realised momentum payoff actually comes from, and the residual that remains is not meaningfully less crash-prone. A second, universe-specific reason to distrust the construction: the "market" proxy is the equal-weight cross-sectional mean of a deeply heterogeneous set (US/JP/HK single stocks alongside bond, gold and EM ETFs), so estimated betas mix genuine equity beta with asset-class and FX effects and are noisy — the Blitz/Huij/Martens setting is a homogeneous single-country equity cross-section with a proper market factor. This result also lines up with a pattern now visible across five separate trials: **every mechanism tried that tilts capital or ranking away from the highest raw-return, higher-beta/higher-vol names has lost Sharpe on this universe** (inverse-vol basket weighting, low-vol tilt, low-vol double sort, square-root weight-spread dampening, and now beta-residualisation), while every mechanism that tilts *toward* them has gained. Treat "de-beta / risk-normalise the signal" as closed absent a genuinely better market factor; do not follow up with the residual-volatility-normalised (t-stat) variant, which sits on the same refuted axis.

## 2026-08-12T01:12:37+00:00 — mom_zscore_weekly_reselect_trim — **REJECT**
- Candidate: `strategies/candidates/mom_zscore_weekly_reselect_trim.py` (family: cross-sectional momentum, trial #33)
- Hypothesis: Re-evaluating the buffered composite-z-score basket's membership and magnitude weights weekly instead of monthly — every other element of trial #28 held identical — raises validation Sharpe above its 1.07, net of 15 bps costs, because the buffer band keeps the extra turnover small while cutting up to three weeks of reaction lag out of the basket's exits from names whose momentum has already broken down.
- Verdict: REJECT — deflated sharpe prob 0.8917 < 0.95 (bar set by 33 total trials)
- Train: sharpe +0.93, ann_ret +18.7%, maxDD -53.7%, turnover 9.4x
- Validation: sharpe +0.99, ann_ret +24.3%, maxDD -31.3%, turnover 15.2x
- Deflated Sharpe prob: 0.8917 (bar from 33 trials)
- Champion validation sharpe at the time: +0.86
- Lesson: **Refuted — the repo's "check the cadence" lesson does not transfer from the exposure overlay to the selection itself; monthly reselection is not a reaction-lag bottleneck.** Versus the otherwise-identical monthly version (trial #28), weekly reselection cut validation Sharpe 1.07 -> 0.99 and more than doubled turnover (7.3x -> 15.2x). The decomposition matters and is the useful part of this result: validation ann_return fell 27.0% -> 24.3%, i.e. by ~2.7pp, while the extra ~7.9x of annual turnover accounts for only about 1.2pp of that at 15 bps/side — so roughly *half* the damage is a worse gross signal, not transaction costs. Reacting faster therefore does not merely cost more, it selects worse: at weekly frequency the composite z-score's ranking near the buffer edges is dominated by short-horizon price noise, so the basket exits names on wiggles that a month-end snapshot averages through. This is the mirror image of the vol-spike-trim finding rather than a contradiction of it — a crash-detection *trigger* benefits from reacting faster than the strategy's own cadence because the thing it detects (a vol regime break) is genuinely fast, whereas the *signal* being ranked is a 6-12 month trend whose true state barely changes week to week, so sampling it more often adds variance and no information. General form worth carrying: match each mechanism's evaluation cadence to the timescale of the phenomenon it measures, not to the strategy's rebalance grid in either direction. Do not retry intermediate selection cadences (biweekly, 10-day) — the gross-return degradation is monotone in speed here and that would be a knob sweep; the untested direction is *slower*, which the next trial takes up.

## 2026-08-12T01:16:42+00:00 — mom_zscore_quarterly_reselect_trim — **REJECT**
- Candidate: `strategies/candidates/mom_zscore_quarterly_reselect_trim.py` (family: cross-sectional momentum, trial #34)
- Hypothesis: Re-evaluating the buffered composite-z-score basket's membership and magnitude weights quarterly instead of monthly — every other element of trial #28 held identical, including the daily-reacting vol-spike trim — raises validation Sharpe above its 1.07, net of 15 bps costs, because a 6-12 month trend signal is sampled with less noise at a slower grid (the mechanism that made weekly reselection lose gross return in trial #33) while annual turnover falls well below 7.3x.
- Verdict: REJECT — deflated sharpe prob 0.9012 < 0.95 (bar set by 34 total trials)
- Train: sharpe +0.87, ann_ret +17.4%, maxDD -65.4%, turnover 2.8x
- Validation: sharpe +1.01, ann_ret +25.4%, maxDD -32.1%, turnover 4.2x
- Deflated Sharpe prob: 0.9012 (bar from 34 trials)
- Champion validation sharpe at the time: +0.86
- Lesson: **Refuted, and together with trial #33 this closes the selection-cadence dimension: monthly is a genuine interior optimum, not a convention.** Quarterly reselection landed at validation Sharpe 1.01 versus monthly's 1.07 (trial #28) and weekly's 0.99 (trial #33) — worse in both directions from monthly. The two failures have *opposite* mechanisms, which is what makes the optimum real rather than a coincidence of costs: weekly lost because it sampled a slow signal too noisily (gross return fell despite reacting faster), whereas quarterly did exactly what it was designed to do on the cost side — turnover fell 7.3x -> 4.2x, worth roughly +0.5pp of annual return at 15 bps/side — and still lost 1.6pp of validation ann_return (27.0% -> 25.4%), i.e. the staleness cost of holding a 3-month-old ranking is about three times the transaction-cost saving it buys. The train split makes the staleness cost even more visible: train Sharpe fell 0.96 -> 0.87 and train maxDD blew out from -54.7% to -65.4%, because a quarterly grid keeps a broken basket through the fast momentum-crash episodes that dominate the pre-2018 history (and the daily vol-spike trim, which was left reacting daily here, only scales exposure — it cannot fix *which* names are held). Practical consequence: turnover reduction is not a promising lever on this strategy at all. The best challenger's 7.3x annual turnover costs about 1.1pp of return, but every mechanism tried to reduce it either loses more gross return than it saves (quarterly grid) or is already priced in (the buffer band itself). Do not revisit selection cadence in either direction, and do not pursue further turnover-reduction-motivated ideas unless they leave the effective holdings unchanged.

## 2026-08-12T01:19:15+00:00 — mom_zscore_daily_voltarget — **REJECT**
- Candidate: `strategies/candidates/mom_zscore_daily_voltarget.py` (family: cross-sectional momentum, trial #35)
- Hypothesis: Scaling the buffered composite-z-score basket's exposure continuously and daily toward a 25% annualised volatility target (capped at 1.0, quantised to 0.05) raises validation Sharpe above the 1.07 of the otherwise-identical binary vol-spike-trim version, net of 15 bps costs, because proportional de-risking uses the same daily volatility information in every regime rather than only above a 1.6x spike threshold.
- Verdict: REJECT — deflated sharpe prob 0.9244 < 0.95 (bar set by 35 total trials)
- Train: sharpe +0.97, ann_ret +19.0%, maxDD -52.9%, turnover 4.6x
- Validation: sharpe +1.07, ann_ret +25.0%, maxDD -30.7%, turnover 8.1x
- Deflated Sharpe prob: 0.9244 (bar from 35 trials)
- Champion validation sharpe at the time: +0.86
