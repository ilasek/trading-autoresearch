...
## 2026-08-15T01:09:12+00:00 — mom_zscore_overlap_daily_trim — **REJECT**
- Candidate: `strategies/candidates/mom_zscore_overlap_daily_trim.py` (family: cross-sectional momentum, trial #33)
- Hypothesis: Holding the best challenger's basket as three overlapping monthly tranches (portfolio = average of the three most recent monthly target-weight vectors, i.e. a 3-month holding period with 1/3 of capital reformed each month), with signal, buffer, weighting and daily vol-spike trim otherwise identical, raises validation Sharpe above 1.07 and cuts turnover below 7.0x, net of 15 bps costs, because it removes single-formation-date luck and structurally limits how much of the book can turn over per month.
- Verdict: REJECT — deflated sharpe prob 0.927 < 0.95 (bar set by 33 total trials)
- Train: sharpe +0.96, ann_ret +19.2%, maxDD -57.9%, turnover 2.7x
- Validation: sharpe +1.08, ann_ret +26.9%, maxDD -28.5%, turnover 4.2x
- Deflated Sharpe prob: 0.927 (bar from 33 trials)
- Champion validation sharpe at the time: +0.86
- Lesson: **Confirmed, and the best result ever recorded in this repo on every axis simultaneously — overlapping formation tranches are a free improvement.** Validation Sharpe 1.08 (vs the previous best 1.07), maxDD -28.5% (vs -30.3%; the first challenger ever to drop below the champion's own -29.9% by a clear margin), and turnover 4.2x (vs 7.0x — a 40% cut). Train-split turnover fell even harder, 7.4x -> 2.7x. Nothing about the signal, buffer, weighting or trim changed; only *when* capital is committed to it. Two independent effects compound here. (1) Cost: only a third of the book can turn over in any month, so the same signal is expressed for materially less friction — consistent with every prior real gain in this repo (buffering, widened breadth) coming from spending less to trade the same idea, and inconsistent with the worry that this line's edge was really a cost artifact. (2) Risk: averaging three formation dates removes the single-date luck that a 100%-monthly-reform basket carries, which shows up as the improved drawdown, and as avg_pos rising 15 -> 25.5 without any change to how many names each individual tranche picks. Note the important asymmetry against the refuted blending results: this dilutes *temporally into the same signal*, whereas `mom_etf_blend`/`mom_buffered_etf_blend` diluted *capital into a weaker second leg* — the dilution tax those established does not apply when the diluting leg is the strategy itself at a different formation date. DSR 0.927 is below the 0.9326 recorded at trial #28, but that comparison is misleading: the bar has risen by five trials since, and 0.927 at trial #33 represents a higher underlying Sharpe than 0.9326 did at #28. Still short of the 0.95 PROMOTE bar. **`mom_zscore_overlap_daily_trim` (val Sharpe 1.08, maxDD -28.5%, turnover 4.2x, DSR 0.927, trial #33) is the new strongest challenger and the base future sessions should build on.**

## 2026-08-15T01:11:11+00:00 — mom_zscore_overlap_concentrated — **REJECT**
- Candidate: `strategies/candidates/mom_zscore_overlap_concentrated.py` (family: cross-sectional momentum, trial #34)
- Hypothesis: Halving the per-tranche basket (hold-14/enter-8 instead of hold-25/enter-15) inside the three-tranche overlapping construction, with signal, weighting and daily vol-spike trim unchanged, raises validation Sharpe above the overlap version's 1.08 while keeping maxDD better than -33%, net of 15 bps costs, because overlap restores at book level the diversification that per-tranche concentration gives up.
- Verdict: REJECT — deflated sharpe prob 0.9209 < 0.95 (bar set by 34 total trials)
- Train: sharpe +1.05, ann_ret +24.7%, maxDD -59.1%, turnover 3.4x
- Validation: sharpe +1.07, ann_ret +30.0%, maxDD -32.8%, turnover 4.1x
- Deflated Sharpe prob: 0.9209 (bar from 34 trials)
- Champion validation sharpe at the time: +0.86
- Lesson: **Refuted — per-tranche conviction and temporal diversification are *not* independent additive levers, and this decomposes where overlap's benefit actually came from.** Halving the per-tranche basket pulled effective book breadth back down (avg_pos 25.5 -> 14.2, i.e. approximately the pre-overlap book as intended), and the result tracked breadth almost exactly: validation Sharpe 1.08 -> 1.07 (flat) while maxDD went -28.5% -> -32.8%, most of the way back to the pre-overlap -30.3%/-36.0% range. Annualised return actually *rose* sharply (26.9% -> 30.0%), so concentration is still adding raw return — it just adds proportionally more volatility and tail risk, which is the same trade the weighting-escalation line ran into two sessions ago. The useful decomposition: **overlap's turnover benefit is temporal (4.1x here vs 4.2x — retained in full even at half the basket size), but its drawdown benefit is breadth-driven (lost as soon as book breadth shrank).** These two halves of the mechanism can be separated, and only the turnover half survives concentration. This also reconciles a prior result that looked contradictory: widening the *nominal* basket from 25/15 to 35/20 left maxDD unchanged at -35.6%, yet overlap's breadth increase cut maxDD by 7 points. The difference is that under magnitude weighting, nominally-added names enter at the bottom of the z-score ranking with near-zero weights, so effective weight concentration barely moves; overlap's extra names enter at *full tranche weight* from an earlier formation date, so they genuinely de-concentrate the book. What matters is effective weight concentration, not the name count — which explains why the earlier breadth experiment was a no-op on drawdown and this one is not. Concentration should now be treated as fully explored in both directions on this signal. `mom_zscore_overlap_daily_trim` (trial #33, val Sharpe 1.08, maxDD -28.5%, turnover 4.2x) stands as the best challenger.

## 2026-08-15T01:13:25+00:00 — mom_zscore_overlap6_daily_trim — **REJECT**
- Candidate: `strategies/candidates/mom_zscore_overlap6_daily_trim.py` (family: cross-sectional momentum, trial #35)
- Hypothesis: Extending the overlapping-tranche holding period from three months to six (portfolio = average of the six most recent monthly target- weight vectors, 1/6 of capital reformed each month), with signal, buffer, weighting and daily vol-spike trim otherwise identical to trial #33, raises validation Sharpe above that version's 1.08 and cuts turnover below 4.2x, net of 15 bps costs, because the turnover and effective-breadth benefits of overlapping formation dates outweigh the decay of a 12-1 momentum signal held six months.
- Verdict: REJECT — deflated sharpe prob 0.9311 < 0.95 (bar set by 35 total trials)
- Train: sharpe +0.98, ann_ret +19.4%, maxDD -56.5%, turnover 2.0x
- Validation: sharpe +1.11, ann_ret +26.9%, maxDD -29.1%, turnover 3.0x
- Deflated Sharpe prob: 0.9311 (bar from 35 trials)
- Champion validation sharpe at the time: +0.86
- Lesson: **Confirmed — the overlap lever is still improving at K=6, and this is the new best result in the repo's history: validation Sharpe 1.11, turnover 3.0x, maxDD -29.1%.** Sharpe rose 1.08 -> 1.11 and turnover fell 4.2x -> 3.0x versus K=3, with maxDD essentially unchanged (-28.5% -> -29.1%) and effective book breadth widening further (avg_pos 25.5 -> 34.2). Train-split turnover is now 2.0x, down from the pre-overlap 7.4x. Notably DSR *rose* (0.927 at trial #33 -> 0.9311 at trial #35) despite two additional trials raising the bar in between — only the second time in this repo's history that a Sharpe improvement has outrun the deflator's bar-raising effect, which is the specific signature the 0.95 gate requires. The economically interesting half of the result is what did *not* happen: holding five sixths of capital on formation signals up to six months stale did not degrade returns at all (ann_ret 26.9%, identical to K=3), so on this universe the 12-1 composite momentum signal's decay over a six-month holding period is slower than the cost and formation-luck savings that a longer holding period buys. Direction on this lever is therefore established as still-positive at K=6, which was the point of running it. **Do not follow this with a K=12 (or K=9) trial next session** — two points already establish direction, and a third to locate a peak would be exactly the parameter sweeping that permanently raises everyone's bar for a knob rather than an idea. If the lever is revisited it should be for a structural reason (e.g. a tranche-weighting scheme other than equal-weight across formation dates), not to find the optimal K.

## 2026-08-15T01:15:24+00:00 — mom_zscore_overlap6_notrim — **REJECT**
- Candidate: `strategies/candidates/mom_zscore_overlap6_notrim.py` (family: cross-sectional momentum, trial #36)
- Hypothesis: Removing the daily-reacting vol-spike exposure trim from the six-tranche overlapping basket, with signal, buffer, weighting and tranche structure otherwise identical to trial #35, leaves validation Sharpe and maxDD statistically indistinguishable from that version's 1.11 and -29.1%, net of 15 bps costs, because a 34-name temporally-averaged book is no longer vol-spiky enough for the 1.6x ratio trigger to fire meaningfully.
- Verdict: REJECT — deflated sharpe prob 0.9137 < 0.95 (bar set by 36 total trials)
- Train: sharpe +0.94, ann_ret +19.4%, maxDD -56.1%, turnover 1.6x
- Validation: sharpe +1.06, ann_ret +26.9%, maxDD -30.3%, turnover 2.6x
- Deflated Sharpe prob: 0.9137 (bar from 36 trials)
- Champion validation sharpe at the time: +0.86
